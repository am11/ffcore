...
ID3D11BlendState *ff::GraphStateCache::GetBlendState(const D3D11_BLEND_DESC& desc)
{
	...
	if (i == INVALID_ITER)
	{
		...
		assertRetVal(SUCCEEDED(_device->GetDX()->CreateBlendState(&desc, &pState)), nullptr);
...
ID3D11DepthStencilState *ff::GraphStateCache::GetDepthStencilState(const D3D11_DEPTH_STENCIL_DESC& desc)
{
	...
	if (i == INVALID_ITER)
	{
		...
		assertRetVal(SUCCEEDED(_device->GetDX()->CreateDepthStencilState(&desc, &pState)), nullptr);
...
ID3D11RasterizerState *ff::GraphStateCache::GetRasterizerState(const D3D11_RASTERIZER_DESC& desc)
{
	...
	if (i == INVALID_ITER)
	{
		...
		assertRetVal(SUCCEEDED(_device->GetDX()->CreateRasterizerState(&desc, &pState)), nullptr);
...
ID3D11SamplerState *ff::GraphStateCache::GetSamplerState(const D3D11_SAMPLER_DESC& desc)
{
	...
	if (i == INVALID_ITER)
	{
		...
		assertRetVal(SUCCEEDED(_device->GetDX()->CreateSamplerState(&desc, &pState)), nullptr);
