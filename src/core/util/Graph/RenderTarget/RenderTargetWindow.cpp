...
namespace ff
{
	...
	private:
		void Destroy();
		bool InitSwapChain(size_t nBackBuffers, bool bFullScreen);
...
{
	static ff::StaticString name(L"CRenderTargetWindow");
...
static ff::PointInt GetMonitorResolution(HWND hwnd, IDXGIAdapterX *pCard, IDXGIOutputX **ppOutput)
{
	...
	if (!ff::ProcessGlobals::Get()->GetGraphicFactory()->GetOutputs(pCard, outputs))
	{
		// just use the desktop monitors
		assertRetVal(ff::ProcessGlobals::Get()->GetGraphicFactory()->GetOutputs(nullptr, outputs), ff::PointInt(0, 0));
	}
...
bool ff::RenderTargetWindow::InitSwapChain(size_t nBackBuffers, bool bFullScreen)
{
	...
	{
		_size = GetClientRect(_hwnd).Size();
	}
	...
	assertRetVal(SUCCEEDED(pFactory->CreateSwapChain(_device->GetDX(), &desc, &_swapChain)), false);
	assertRetVal(SUCCEEDED(pFactory->MakeWindowAssociation(_hwnd, DXGI_MWA_NO_WINDOW_CHANGES)), false);
...
void ff::RenderTargetWindow::Reset()
{
	...
	verify(InitSwapChain(desc.BufferCount, bFullScreen));
}
...
{
	return RectInt(0, 0, _size.x, _size.y);
}
...
void ff::RenderTargetWindow::Present(bool bVsync)
{
	...
	UINT nSync  = (bVsync && !_occluded) ? 1 : 0;
...
bool ff::RenderTargetWindow::CanSetFullScreen() const
{
	...
	{
		ProcessGlobals::Get()->GetGraphicFactory()->GetOutputs(_device->GetAdapter(), outputs);
	}
	...
	case SIZE_RESTORED:
		{
			...
			if (bResize)
			{
				...
				PointInt size = GetClientRect(_hwnd).Size();
