...
{
	const ff::SpriteData &data = pSprite->GetData();
...
{
	const ff::SpriteData& data1 = sprite1._sprite->GetData();
	const ff::SpriteData& data2 = sprite2._sprite->GetData();
	...
	ff::Vector<ff::ComPtr<ff::IGraphTexture>> &textures)
{
	for (size_t i = 0; i < textureInfos.Size(); i++)
	{
		...
		pRenderTarget->Clear(&XMFLOAT4(0, 0, 0, 0));
...
static bool CopyOptimizedSprites(ff::Vector<OptimizedSpriteInfo> &spriteInfos, ff::Vector<ff::ComPtr<ff::IGraphTexture>> &textures)
{
	for (size_t i = 0; i < spriteInfos.Size(); i++)
	{
		...
		const ff::SpriteData &data = sprite._sprite->GetData();
		...
		sprite._sprite->CopyTo(pTexture, sprite._destRect.TopLeft());
	}
	...
	ff::Vector<ff::ComPtr<ff::IGraphTexture>>& finalTextures)
{
	...
	{
		ff::IGraphTexture* pTexture = textures[i];
		ff::ComPtr<ff::IGraphTexture> pNewTexture;

		assertRetVal(pTexture->Convert(format, nMipMapLevels, &pNewTexture), false);
		finalTextures.Push(pNewTexture);
	}
...
bool ff::OptimizeSprites(ISpriteList *pSprites, DXGI_FORMAT format, size_t nMipMapLevels, ISpriteList **ppOptimizedSprites)
{
	...
	assertRetVal(ConvertOptimizedTextures(format, nMipMapLevels, textures, finalTextures), false);

	// Create final sprites
	for (size_t i = 0; i < sprites.Size(); i++)
	{
		OptimizedSpriteInfo& sprite = sprites[i];
		const SpriteData&    data   = sprite._sprite->GetData();
