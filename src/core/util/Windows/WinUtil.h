...
namespace ff
{
	typedef Windows::UI::Core::CoreWindow ^PWND;
	typedef Platform::Agile<Windows::UI::Core::CoreWindow> MPWND;
	UTIL_API PWND MemberToWindow(MPWND window);

	UTIL_API bool HandleMessages(void *ignored = nullptr);
	UTIL_API void WaitForMessage(void *ignored = nullptr, DWORD ignored2 = INFINITE);
	UTIL_API bool GotQuitMessage();
	UTIL_API void PostQuitMessage();
	UTIL_API bool IsShowingModalDialog();

	UTIL_API int MessageBox(Windows::UI::Core::CoreWindow ^parent, ff::StringRef text, ff::StringRef title, int options);
	UTIL_API SHORT GetKeyState(int vk);
