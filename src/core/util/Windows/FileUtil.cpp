...
#if METRO_APP
	_file = ::CreateFile2(szCheckPath.c_str(), GENERIC_READ, FILE_SHARE_READ, OPEN_EXISTING, nullptr);
#else
	_file = ::CreateFile(szCheckPath.c_str(), GENERIC_READ, FILE_SHARE_READ, nullptr, OPEN_EXISTING, 0, nullptr);
#endif
...
bool ff::MemMappedFile::OpenWrite(HANDLE hFile, size_t nNewSize)
{
	...
	nNewSize = (nNewSize != INVALID_SIZE) ? nNewSize : GetFileSize(hFile);
...
bool ff::MemMappedFile::OpenRead(HANDLE hFile)
{
	...
	size_t nSize = GetFileSize(hFile);
	if (!nSize)
	{
		return false;
	}
...
ff::String ff::GetKnownDirectory(REFGUID id, bool bCreate)
{
	...
	assertRetVal(SUCCEEDED(SHGetKnownFolderPath(id, bCreate ? KF_FLAG_CREATE : 0, nullptr, &szPath)), szReturnPath);
