...
ff::PWND ff::MemberToWindow(MPWND wnd)
{
	return wnd.Get();
}

int ff::MessageBox(CoreWindow ^parent, ff::StringRef text, ff::StringRef title, int options)
{
	// MessageDialog ^dialog = ref new MessageDialog(text.pstring(), title.pstring());
	// dialog->ShowAsync();
...
SHORT ff::GetKeyState(int vk)
{
	CoreVirtualKeyStates ks = GetMainThreadWindow()->GetKeyState((Windows::System::VirtualKey)vk);
	return ((ks & CoreVirtualKeyStates::Down) == CoreVirtualKeyStates::Down) ? 0x8000 : 0x0000;
}
...
bool ff::HandleMessages(void *ignored)
{
	noAssertRetVal(GetMainThreadWindowXaml() == nullptr, true); // XAML will handle its own messages
	GetMainThreadDispatcher()->ProcessEvents(CoreProcessEventsOption::ProcessAllIfPresent);
	return true;
}


void ff::WaitForMessage(void *ignored, DWORD ignored2)
{
	noAssertRet(GetMainThreadWindowXaml() == nullptr); // XAML will handle its own messages
	GetMainThreadDispatcher()->ProcessEvents(CoreProcessEventsOption::ProcessOneAndAllPending);
}
...
ff::PWND ff::MemberToWindow(MPWND window)
{
	return window;
}
...
static ff::ProgramShutdown Cleanup([]()
{
