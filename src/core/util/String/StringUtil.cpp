...
#if METRO_APP
	const Module *module = ProcessGlobals::Get()->GetModules().Get(hInstance);
	assertRetVal(module, str);
	return module->GetString(nID);
#else
	wchar_t buf[1024];
...
{
#if METRO_APP
	String sz = LoadString(hInstance, nID);
	return sz.bstr();
#else
	BSTR ret = nullptr;
	wchar_t buf[1024];
