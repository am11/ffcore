...
ff::Vector<T, StackSize, Allocator>::Vector()
	: _data(reinterpret_cast<T *>(_stack))
	, _alloc(StackSize)
	, _size(0)
{
...
ff::Vector<T, StackSize, Allocator>::Vector(MyType &&rhs)
	...
	, _size(rhs._size)
{
	if (rhs._data == reinterpret_cast<T *>(rhs._stack))
	{
	...
	rhs._data  = reinterpret_cast<T *>(rhs._stack);
...
{
	if (_size < _alloc && _data != reinterpret_cast<T *>(_stack))
	{
		if (_size == 0)
		{
			ReleaseData();
		...
		else
		{
			...
			_data  = data;
			_size  = size;
			_alloc = (_data == reinterpret_cast<T *>(_stack)) ? StackSize : _size;
		}
	}
}
...
{
	assert(data && pos >= 0 && count >= 0 && pos <= _size);
...
{
	if (_data != reinterpret_cast<T *>(_stack))
	{
		if (_alloc)
		{
