...
namespace ff
{
	...
	public:
		...
		size_t  Size() const;
		T      *Delete(const T &obj); // returns the object after the deleted object
		void    DeleteFirst();
		void    DeleteLast();
		void    Clear();
		bool    IsEmpty() const;
	...
	template<typename T>
	T *List<T>::Delete(const T &obj)
	{
		ListItem *item = ItemFromObject(&obj);
		ListItem *nextItem = item->_next;
	...
	{
		return const_cast<ListItem *>(reinterpret_cast<const ListItem *>(obj));
	}
