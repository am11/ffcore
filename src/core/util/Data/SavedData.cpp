...
namespace ff
{
	...
	public:
		...
		virtual void   SetDataLoader(IAsyncDataLoader *pLoader) override;
		virtual bool   GetDataLoader(IAsyncDataLoader **ppLoader) override;
	...
	private:
		...
		ComPtr<IAsyncDataLoader> _loader;
...
void ff::CSavedData::FlushAsyncWork()
{
	if (_loader)
	{
		verify(_loader->Wait(this));
		assert(!_loader);
	}
}
...
void ff::CSavedData::SetDataLoader(IAsyncDataLoader *pLoader)
{
	_loader = pLoader;
}

bool ff::CSavedData::GetDataLoader(IAsyncDataLoader **ppLoader)
{
	if (ppLoader)
	{
		*ppLoader = _loader.Detach();
	}
